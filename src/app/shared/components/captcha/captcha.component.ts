...
export class CaptchaComponent implements AfterViewInit, OnInit, OnChanges {
                ...
                .subscribe(
                    (response) => {
                        ...
                        if (this.isValidPayload(payload)) {
                            ...
                            this.errorVerifyAnswer = this.createErrorTextLine(
                                response
                            );
                        }
                    },
                    (error) => {
                        ...
                        console.log(
                            'Error response from verifying user answer: %o',
                            error
                        );
                    }
                );
    ...
    private isValidPayload(payload) {
        ...
        if (!payload) {
            ...
            if (!hasValueProp) {
                console.error(
                    "payload must have its own property named 'valid'"
                );
                return false;
            } else {
                return true;
    ...
    public getNewCaptcha(errorCase: any) {
        ...
        this.dataService.fetchData(this.apiBaseUrl, this.nonce).subscribe(
            ...
            (error) => {
                ...
                console.log(
                    'Error esponse from fetching CAPTCHA text: %o',
                    error
                );
                this.cd.detectChanges();
            }
