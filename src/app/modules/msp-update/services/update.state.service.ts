...
export class UpdateStateService {
  ...
  private formsStatus = new BehaviorSubject<FormStatusUpdate>(
    {
      ...
      signingAuthority: { hasData: false, add: false, remove: false, update: false },
      mspAccessAdministrators: { hasData: false, add: false, remove: false, update: false },
  ...
  private getFormStatus(form: AddRemoveUpdate): FormStatusAddRemoveUpdate {
    ...
    const formStatus: FormStatusAddRemoveUpdate = { hasData: false, add: false, remove: false, update: false };

    const formsRemove = form.remove ? form.remove.get('arrayOfForms') as FormArray | null : null;
    formStatus.remove = formsRemove && formsRemove.controls.length > 0 ? true : false;

    const formsAdd = form.add ? form.add.get('arrayOfForms') as FormArray | null : null;
    formStatus.add = formsAdd && formsAdd.controls.length > 0 ? true : false;

    const formsEdit = form.update ? form.update.get('arrayOfForms') as FormArray | null : null;
    formStatus.update = formsEdit && formsEdit.controls.length > 0 ? true : false;

    formStatus.hasData = formStatus.add || formStatus.remove || formStatus.update ? true : false;
